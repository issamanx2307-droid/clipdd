...
export default function RegisterPage() {
  ...
  const loginWithGoogle = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (!res.ok) { setError(data.detail || 'Google Sign-In ล้มเหลว'); return }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setError(data.email?.[0] || data.password?.[0] || data.detail || 'เกิดข้อผิดพลาด'); return }
      localStorage.setItem('cd_token', data.token)
      localStorage.setItem('cd_user', JSON.stringify(data.user))
      window.location.href = '/create'
        ...
        <div className={styles.card}>
          ...
          <form onSubmit={handleSubmit} className={styles.form}>
            <div className={styles.field}>
              ...
              <input className={styles.input} type="text" placeholder="ชื่อร้านหรือชื่อคุณ"
